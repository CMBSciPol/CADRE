...
from .constraints import GaussianConstraint
...
class ActiveSetState(NamedTuple):
    ...
    # KKT termination: projected gradient norm (internal space, pre-rescaling)
    gnorm_proj: Scalar
    # Langevin noise PRNG key
    key: jax.Array
...
def active_set(
    ...
    reset_direction_fn: Callable[[optax.OptState], optax.OptState] | None = None,
    noise_temperature: float = 0.0,
    noise_decay: float = 1e-3,
    noise_key: int = 0,
    ...
    * ``reset_direction_fn`` — called when ``constraints_released`` is True to
      reset the inner direction-solver state (e.g. zero AdaBelief moments or
      invalidate L-BFGS history). Each solver supplies a typed reset function;
      ``None`` disables the reset.
    * ``noise_temperature`` / ``noise_decay`` — preconditioned Langevin noise.
      Noise scale is ``T·exp(-decay·count) / (‖step‖ + ε)``: large when the
      line-search step is small (stuck), small when the step is large
      (already making progress). The kick is applied **after** the line
      search and is therefore not gated by Wolfe conditions — it can
      actually move the iterate uphill and escape a local minimum.
      ``BestSoFarMinimiser`` keeps the lowest-f point seen, so noise can
      only help (never hurt) the reported result. The kick is also capped
      at half the distance to the nearest bound to preserve feasibility.
      Injected only on free parameters. ``noise_temperature=0.0`` (default)
      disables it entirely.
    * ``gaussian_prior`` — adds ``(x − loc) / scale²`` to the physical gradient
      (ridge regularization toward the prior mean). Mutually exclusive with
      hard box bounds in spirit but mechanically additive.

    References:
        Kabalan et al. (2025), arXiv:2604.08463 — ADABK / AdaTopK active set
        Ma et al. (2015), arXiv:1506.04696 — preconditioned SGLD
        Welling & Teh (2011), ICML — stochastic gradient Langevin dynamics
    """
    ...
    def init_fn(params: PyTree[Float[Array, " P"]]) -> ActiveSetState:
        ...
        return ActiveSetState(
            ...
            key=jax.random.PRNGKey(noise_key),
        )
    ...
    def update_fn(
        ...
        **kwargs: Any,
        ...
        # NOTE: When a Gaussian prior is requested via ``minimize(constraints=...)``,
        # ``fn`` is wrapped at the top level (in ``minimize.py``) to include the
        # prior log-likelihood. This keeps ``grads``, ``value``, ``value_fn`` and
        # the optimistix BestSoFar tracker all consistent with the MAP objective.
        # The ``gaussian_prior`` argument is therefore intentionally inert here.
        _ = gaussian_prior
        ...
        # --- 3c. Reset direction state on pivot change ---
        # When the active set changes, the inner solver's history is invalidated.
        # Each solver supplies its own typed reset_direction_fn (AdaBelief moments,
        # L-BFGS curvature pairs + EMA trace, etc.).
        if reset_direction_fn is not None:
            base_dir_state = jax.lax.cond(
            ...
            base_dir_state = state.direction_state
        ...
        current_dir_state = otu.tree_where(
            should_rescale,
            _rescale_adam_state(base_dir_state, 1.0 / safe_gnorm),
            base_dir_state,
        )
        ...
        # --- 8b. Post-line-search Langevin perturbation (free-param subspace only) ---
        # The line search guarantees a descent step in ``final_update_int``; we
        # then add an isotropic Gaussian "kick" whose scale is INVERSELY
        # proportional to the line-search step magnitude. When the algorithm is
        # making good progress (large step) the kick is small and the
        # line-search step dominates; when stuck (tiny step) the kick can
        # actually move the iterate to a different basin. The kick is applied
        # AFTER the line search precisely so it is not gated by Wolfe
        # conditions; ``BestSoFarMinimiser`` ensures we never report a worse
        # final loss than the deterministic descent path.
        # References:
        #   Ma et al. (2015), arXiv:1506.04696 — preconditioned SGLD
        #   Welling & Teh (2011), ICML — SGLD
        if noise_temperature > 0.0:
            new_key, subkey = jax.random.split(state.key)
            effective_T = noise_temperature * jnp.exp(
                -noise_decay * state.count.astype(jnp.float32)
            )
            ls_norm = otu.tree_norm(final_update_int)
            noise_scale = effective_T / (ls_norm + 1e-8)
            # Cap noise so a single kick cannot move further than half the
            # distance to the nearest bound; preserves feasibility.
            noise_scale = jnp.minimum(noise_scale, 0.5 * spe)
            leaves, treedef = jtu.tree_flatten(final_update_int)
            keys = jax.random.split(subkey, len(leaves))
            piv_leaves = jtu.tree_leaves(final_pivot)
            new_leaves = [
                jnp.where(
                    pl == 0,
                    leaf + jax.random.normal(k, leaf.shape) * noise_scale,
                    leaf,
                )
                for leaf, k, pl in zip(leaves, keys, piv_leaves)
            ]
            final_update_int = jtu.tree_unflatten(treedef, new_leaves)
        else:
            new_key = state.key
